...
def _add_market_relative_features(df, date_col='日期'):
    """添加市场相对特征（需要在所有股票合并后调用）"""
    df = df.copy()

    if '涨跌幅' in df.columns:
        df['market_return'] = df.groupby(date_col)['涨跌幅'].transform('mean')
        df['market_return_std'] = df.groupby(date_col)['涨跌幅'].transform('std')
        df['excess_return'] = df['涨跌幅'] - df['market_return']
        df['return_zscore'] = df['excess_return'] / (df['market_return_std'] + 1e-12)
        df.drop(columns=['market_return', 'market_return_std'], inplace=True)

    if '成交量' in df.columns:
        df['market_volume'] = df.groupby(date_col)['成交量'].transform('mean')
        df['volume_relative'] = df['成交量'] / (df['market_volume'] + 1e-12)
        df.drop(columns=['market_volume'], inplace=True)

    if '换手率' in df.columns:
        df['market_turnover'] = df.groupby(date_col)['换手率'].transform('mean')
        df['turnover_relative'] = df['换手率'] / (df['market_turnover'] + 1e-12)
        df.drop(columns=['market_turnover'], inplace=True)

    if '振幅' in df.columns:
        df['market_amplitude'] = df.groupby(date_col)['振幅'].transform('mean')
        df['amplitude_relative'] = df['振幅'] / (df['market_amplitude'] + 1e-12)
        df.drop(columns=['market_amplitude'], inplace=True)

    if '涨跌幅' in df.columns:
        df['return_rank'] = df.groupby(date_col)['涨跌幅'].rank(pct=True)
    if '成交量' in df.columns:
        df['volume_rank'] = df.groupby(date_col)['成交量'].rank(pct=True)
    if '换手率' in df.columns:
        df['turnover_rank'] = df.groupby(date_col)['换手率'].rank(pct=True)
...
def _build_label_and_clean(processed, drop_small_open=True):
    ...
    # 计算个股5日收益率
    processed['raw_return'] = (processed['open_t5'] - processed['open_t1']) / (processed['open_t1'] + 1e-12)

    # 计算当日市场平均收益率，然后用超额收益作为标签
    processed['market_return'] = processed.groupby('日期')['raw_return'].transform('mean')
    processed['label'] = processed['raw_return'] - processed['market_return']

    processed = processed.dropna(subset=['label'])
    processed.drop(columns=['open_t1', 'open_t5', 'raw_return', 'market_return'], inplace=True)
    return processed
...
def _preprocess_common(df, stockid2idx, desc, drop_small_open=True):
    ...
    num_processes = min(config.get('num_processes', 10), mp.cpu_count())
    with mp.Pool(processes=num_processes) as pool:
        processed_list = list(tqdm(pool.imap(feature_engineer, groups), total=len(groups), desc=desc))

    processed = pd.concat(processed_list).reset_index(drop=True)

    # 添加市场相对特征（需要在所有股票合并后计算）
    print(">>> 添加市场相对特征...")
    processed = _add_market_relative_features(processed, date_col='日期')
...
class ApproxNDCGLoss(nn.Module):
    """
    可微分的 NDCG 损失函数，直接优化排序指标。
    使用 softmax 近似排名，使得梯度可以反向传播。
    """
    ...
    def forward(self, y_pred, y_true):
        ...
        # 1. 计算近似排名（可微分）
        # approx_ranks[i,j] ≈ 1 + sum_{l!=j} sigmoid((s_l - s_j) / temp)
        pred_diff = y_pred.unsqueeze(2) - y_pred.unsqueeze(1)  # [B, N, N]
        approx_ranks = 1.0 + torch.sum(
            torch.sigmoid(pred_diff / self.temperature),
            dim=2
        )  # [B, N]

        # 2. 计算 DCG
        # 将真实收益转换为相关性分数（归一化到 [0, 1]）
...
class CombinedRankingLoss(nn.Module):
    """
    组合损失：WeightedRankingLoss + ApproxNDCGLoss
    """
    def __init__(self, temperature=1.0, k=5, weight_factor=2.0, pairwise_weight=1.0,
                 base_weight=1.0, ndcg_weight=0.5):
        super(CombinedRankingLoss, self).__init__()
        self.weighted_loss = WeightedRankingLoss(
            temperature=temperature, k=k, weight_factor=weight_factor,
    ...
    def forward(self, y_pred, y_true):
        weighted = self.weighted_loss(y_pred, y_true)
        ndcg = self.ndcg_loss(y_pred, y_true)
        return weighted + self.ndcg_weight * ndcg
...
def main():
    ...
    # 7. 损失函数和优化器（使用组合损失：WeightedRankingLoss + ApproxNDCGLoss）
    criterion = CombinedRankingLoss(
        k=config.get('loss_k', 5),
    ...
    scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=config.get('scheduler_start_factor', 1.0), end_factor=config.get('scheduler_end_factor', 0.2), total_iters=config['num_epochs'])
