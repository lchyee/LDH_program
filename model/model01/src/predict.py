...
def preprocess_predict_data(df, stockid2idx):
    ...
    print(f'使用 {num_processes} 个进程进行特征工程')
...
def main():
    ...
    model_output_dir = config.get('model_output_dir', './model/model01/output')
    ...
    # result.csv：前5只，仅 stock_id + weight（符合赛事评分规范）
    # 修改为输出 Top-50，供顶层 vote.py 投票使用
    TOP_N = 50
    # 确保不超过实际预测的股票总数
    TOP_N = min(TOP_N, len(ranked_stock_ids))

    topN_ids = ranked_stock_ids[:TOP_N]
    result_df = pd.DataFrame({
        'stock_id': topN_ids,
        'rank': list(range(1, TOP_N + 1))  # 必须添加 rank 列供投票脚本计算
    })
    result_path = os.path.join(model_output_dir, 'result.csv')
    result_df.to_csv(result_path, index=False)

    # result-all.csv：前10只，含 stock_id、weight、rank、score（量化依据）
    top_ids = ranked_stock_ids[:top_k]
    ...
    result_all_path = os.path.join(model_output_dir, 'result-all.csv')
    result_all_df.to_csv(result_all_path, index=False)

    print(f'预测日期: {latest_date.date()}')
    print(f'参与排序股票数: {len(ranked_stock_ids)}')
    print(f'Top-5 已写入: {result_path}')
    print(f'Top-{top_k} 详情已写入: {result_all_path}')
