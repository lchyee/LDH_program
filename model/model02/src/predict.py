"""
iTransformer 推理脚本 (Model02) - 全量排名输出 + 历史上下文拼接增强版
"""
import os
import sys
import joblib
...
def main():
    ...
    # 【修复数据泄露】：只读取训练集，不读取测试集
    print(">>> 正在加载训练数据...")
    raw_df = pd.read_csv(TRAIN_PATH, dtype={'股票代码': str})
    ...
    print(f"✓ 预测完成！共输出 {len(result_df)} 只股票的全景排名，已保存在: {result_path}")
    return 0
