"""
iTransformer 训练脚本 (Model02) - 横截面排序进化版
修复了原版 MSELoss 无法排序、以及 DataLoader 打乱时间截面的致命问题。
"""
import os
import sys
import joblib
...
class WeightedRankingLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        # y_pred, y_true shape: [batch(1), num_stocks]
...
# 【核心修复2】横截面数据集。保证模型看到的是“同一天的所有股票”，而不是跨时空瞎比。
...
def main():
    set_seed(42)
    print("========== 开始训练: iTransformer (Ranking Loss 进化版) ==========")
    ...
    # 【必须先按日期排序】防止未来数据泄露和 shift 错乱
    df = df.sort_values(['股票代码', '日期']).reset_index(drop=True)

    print(">>> 并行提取 158 个Alpha特征...")
    ...
    criterion = WeightedRankingLoss(k=50)  # 关注前50名
    ...
    print(f"✓ 训练完成！最佳 epoch: {best_epoch}, 最佳验证 Loss: {best_val_loss:.4f}")
