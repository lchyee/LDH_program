"""
MICN 推理脚本 (Model07) - 全量排名 + 历史拼接增强版
"""
import os
import sys
import joblib
...
from shared_components.feature_utils import add_market_relative_features


def main():
    print("========== 开始预测: MICN (Model07) ==========")
    ...
    # 【与训练对齐】：补上市场相对特征（训练时启用了这些横截面特征，推理必须同口径计算）
    print(">>> 添加市场相对特征...")
    raw_df = add_market_relative_features(raw_df, date_col='日期')
    ...
    print(f"✓ [Model07] 预测完成！共输出 {len(result_df)} 只股票的全景排名。")
    return 0
