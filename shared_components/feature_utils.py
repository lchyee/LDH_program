...
def add_market_relative_features(df, date_col='日期'):
    """
    添加市场相对特征（需要在所有股票合并后调用）。
    这些特征衡量个股相对于当日市场整体的表现。
    """
    df = df.copy()
...
def engineer_features(df):
    ...
    for w in windows:
        ...
        actual_w = len(close.index[:w])  # 【核心修复】：动态获取实际数据长度
        time_period_series = pd.Series(range(actual_w), index=close.index[:actual_w])
